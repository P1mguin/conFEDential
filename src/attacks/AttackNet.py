...
class AttackNet(nn.Module):
	...
	def _initialize_components(self, gradient_shapes, activation_shapes, metrics_shapes, label_shape):
		...
		self._initialize_activation_component(activation_shapes[:-1])
	...
	def _initialize_activation_component(self, activation_shapes):
		activation_components = [[nn.Flatten(start_dim=1)] for _ in activation_shapes]
	...
	def _initialize_encoder_component(self):
		...
		concatenated_size += get_output_size(self.loss_component) * global_rounds
		for metric_components in self.metric_components.values():
			for metric_component in metric_components:
				concatenated_size += get_output_size(metric_component) * global_rounds
		for activation_component in self.activation_components:
			concatenated_size += get_output_size(activation_component) * global_rounds
		for gradient_component in self.gradient_components:
			concatenated_size += get_output_size(gradient_component) * global_rounds
